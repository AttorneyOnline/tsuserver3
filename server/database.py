...
class Database:
    ...
    def log_ic(self, client, room, showname, message):
        """Log an IC message."""
        event_logger.info(f'[{room.name}] {showname}/{client.char_name}' +
                          f'/{client.name} ({client.ipid}): {message}')
        with self.db as conn:
            conn.execute(dedent('''
                INSERT INTO ic_events(ipid, room_name, char_name, ic_name,
                    message) VALUES (?, ?, ?, ?, ?)
                '''), (client.ipid, room.name, client.char_name,
                    showname, message))
    ...
    def log_room(self, event_subtype, client, room, message=None, target=None):
        ...
        event_logger.info(f'[{room.name}] {client.char_name}' +
        ...
        with self.db as conn:
            conn.execute(dedent('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                '''), (ipid, room.name, char_name, ooc_name,
