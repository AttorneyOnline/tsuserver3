...
def ooc_cmd_disemvowel(client, arg):
    ...
    if targets:
        for c in targets:
            logger.log_server('Disemvowelling {}.'.format(c.ip), client)
            logger.log_mod('Disemvowelling {}.'.format(c.ip), client)
            c.disemvowel = True
        client.send_ooc('Disemvowelled {} existing client(s).'.format(
            len(targets)))
    else:
        client.send_ooc('No targets found.')
...
def ooc_cmd_undisemvowel(client, arg):
    ...
    if targets:
        for c in targets:
            logger.log_server('Undisemvowelling {}.'.format(c.ip),
                              client)
            logger.log_mod('Undisemvowelling {}.'.format(c.ip), client)
            c.disemvowel = False
        client.send_ooc(
...
def ooc_cmd_shake(client, arg):
    ...
    if targets:
        for c in targets:
            logger.log_server('Shaking {}.'.format(c.ip), client)
            logger.log_mod('Shaking {}.'.format(c.ip), client)
            c.shaken = True
        client.send_ooc('Shook {} existing client(s).'.format(
            len(targets)))
    else:
        client.send_ooc('No targets found.')
...
def ooc_cmd_unshake(client, arg):
    ...
    if targets:
        for c in targets:
            logger.log_server('Unshaking {}.'.format(c.ip), client)
            logger.log_mod('Unshaking {}.'.format(c.ip), client)
            c.shaken = False
        client.send_ooc('Unshook {} existing client(s).'.format(
            len(targets)))
    else:
        client.send_ooc('No targets found.')
